...
;         
 ...
 WHERE COD_EMPR IN (SELECT COD_EMPR FROM USR_EMPR WHERE COD_USR =2) AND LCT_TIPO = 'Receita' AND LCT_STATUSLANC = 'A Pagar - Receber' 
 AND  DATE_FORMAT(LCT_DTVENC, '%Y-%m-%d') <= '2017-11-31') AS RECEITA, 
	(SELECT SUM(LCT_VLRTITULO) from LANCAMENTO  INNER JOIN CONTA ON LANCAMENTO.CNT_COD = CONTA.CNT_COD INNER JOIN EMPRESA ON EMP_COD = COD_EMPR
WHERE COD_EMPR IN (SELECT COD_EMPR FROM USR_EMPR WHERE COD_USR =2) AND LCT_TIPO = 'Despesa' AND LCT_STATUSLANC = 'A Pagar - Receber'
AND DATE_FORMAT(LCT_DTVENC, '%Y-%m-%d') <= '2017-11-31') AS DESPESA, 
...
(SELECT IFNULL((SELECT SUM(LCT_VLRTITULO) from LANCAMENTO INNER JOIN CONTA ON LANCAMENTO.CNT_COD = CONTA.CNT_COD 
                INNER JOIN EMPRESA ON EMP_COD = COD_EMPR  WHERE COD_EMPR IN (SELECT COD_EMPR FROM USR_EMPR WHERE COD_USR = 2) AND LCT_STATUSLANC = 'Pago - Recebido' AND LCT_TIPO = 
                ...
                ON EMP_COD = COD_EMPR  WHERE COD_EMPR IN (SELECT COD_EMPR FROM USR_EMPR WHERE COD_USR = 2) AND LCT_STATUSLANC = 
...
SELECT USR_PONTUACAO FROM USUARIO WHERE USR_COD = 2;